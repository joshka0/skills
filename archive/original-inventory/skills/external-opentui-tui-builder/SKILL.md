---
name: external-opentui-tui-builder
description: >
  Expert terminal UI design and implementation for OpenTUI. Use when building,
  reviewing, or polishing command-line interfaces, terminal dashboards, interactive
  CLI tools, keyboard-driven workflows, forms, selectors, panels, logs, chat-like
  interfaces, command palettes, status bars, split-footer assistants, code/diff
  viewers, scrollable panes, and React/Solid/Core OpenTUI apps.
  Triggers on: OpenTUI, TUI, terminal UI, CLI UI, terminal dashboard, keyboard
  shortcuts, focus management, alternate screen, split footer, command palette,
  terminal form, terminal selector, terminal layout, terminal colors, terminal
  accessibility, terminal animations, @opentui/core, @opentui/react, createCliRenderer,
  Box, Text, Input, Select, ScrollBox, Code, Diff, Markdown, useKeyboard,
  useTerminalDimensions, useTimeline.
---
