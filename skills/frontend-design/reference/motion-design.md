...
/* For legacy codebases, reduce decorative motion rather than globally nuking everything */
@media (prefers-reduced-motion: reduce) {
  .parallax,
  .marquee,
  .decorative-motion {
...
**What to preserve**: Functional animations like progress bars, loading spinners (slowed down), and focus indicators should still work—just without spatial movement. Avoid global `0.01ms` overrides unless you are auditing a legacy UI and can verify nothing essential breaks.
