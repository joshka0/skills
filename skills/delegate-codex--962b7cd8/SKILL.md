---
name: delegate-codex
description: "Quick single-task delegation to Codex CLI agent. No Zellij, no layout — just runs codex exec in background. Use for: delegate to codex, run codex, quick codex task, codex exec, single codex agent, background codex, delegate task."
---

# Delegate Codex Skill

Run a single Codex CLI agent in the background. Lightweight alternative to `/swarm` — no Zellij, no layout files, just `codex exec` with worktree isolation.
...
Poll periodically using the Bash tool or `TaskOutput`:
