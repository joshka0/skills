---
name: codex-goal
description: "Create, refine, run, and monitor Codex CLI /goal objectives for persistent autonomous work. Use when the user asks for a Codex goal prompt, long-running agent objective, Ralph-loop style plan-act-test-review workflow, /goal setup, goal verification criteria, phased autonomous refactors, feature implementation, bug hunts, test backfills, or reducing /goal loops and token waste."
---

# Codex Goal

Use this skill to turn broad engineering work into a verifiable Codex CLI
`/goal` objective stored in a goal file. A good goal behaves like a contract:
clear scope, explicit constraints, measurable completion criteria, and focused
verification.

For expanded templates, monitoring guidance, and examples, read
[references/guide.md](references/guide.md) when crafting a complex or multi-day
goal.
...
Avoid `/goal` for vague requests, ambiguous product decisions, risky production
operations, secrets work, or tasks where a human must choose direction at every
step.
...
3. Write the objective to a file named `<task>_goal.md`. Prefer
   `docs/goals/<task>_goal.md` when the repo has docs, otherwise put it near the
   current planning artifacts.
4. Start with `/goal docs/goals/<task>_goal.md`,
   `/goal <task>_goal.md`, or `/goal create <task>_goal.md` depending on the
   CLI build.
...
Then include short sections for why the goal is verifiable, monitoring notes,
and stop conditions.
