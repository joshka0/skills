...
Fetch these resources before generating or validating workflow files. Use the
fetch script in this skill's `scripts/` directory; it caches responses using
...
The validator fetches the latest schema and checks the YAML structure. Fix any
reported errors before considering the workflow complete.

## Answering Questions

When users ask about available options (job types, triggers, runner types, etc.),
fetch the schema and derive the answer from it rather than relying on potentially
