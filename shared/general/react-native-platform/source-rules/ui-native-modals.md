...
Prefer native `<Modal>` with `presentationStyle="formSheet"` or React
Navigation v7's native form sheet for simple modal flows that benefit from
platform behavior. Native modals have built-in gestures, accessibility, and
better performance. Use a JS bottom sheet when the product needs custom snap
points, nested gestures, highly branded interaction, or cross-platform behavior
native sheets cannot express.
