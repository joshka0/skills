...
**Never `outline: none` without replacement.** It's an accessibility violation. Use a visible `:focus` fallback, then refine it with `:focus-visible`:
...
/* Hide the fallback only when the browser supports :focus-visible */
